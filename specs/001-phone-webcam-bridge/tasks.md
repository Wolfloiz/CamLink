...
- [x] T066 [P] Instalador Linux: `installer/linux/install.sh` (deps, udev rules, modules-load.d) + bundle `.deb`/AppImage + `installer/linux/PKGBUILD` (AUR), jar do fork embutido (FR-024) — **concluído (2026-08-20)**. **Desvio consciente do research R3**: a escalação prevista era `pkexec` + policy polkit, mas o código nunca implementou pkexec (`v4l2.rs` chama `v4l2loopback-ctl` direto) — o que funcionava em bancada era a regra udev criada à mão pelo usuário. Formalizada em vez de trocada: `installer/linux/99-camlink-v4l2loopback.rules` (`KERNEL=="v4l2loopback", GROUP="video", MODE="0660"`) + usuário no grupo `video`, sem privilégio em nenhum caminho do app e sem prompt de senha por fonte iniciada. `camlink-v4l2loopback.conf` (modules-load.d) só carrega o módulo e `modprobe-camlink.conf` força `devices=0` — o oposto do `/etc/modprobe.d` legado da máquina de dev (que ainda dizia `CamDroidLink` e pré-criava `video_nr=2,3,4`, colidindo com a alocação dinâmica e deixando webcam fantasma no OBS com o app fechado). **Estratégia híbrida de dependências**: `.deb` declara `Depends: adb, ffmpeg, v4l-utils, v4l2loopback-dkms (>= 0.13)`; AppImage leva adb + ffmpeg vendorizados (`vendor.sh`, espelha o `vendor.ps1` do T067) porque precisa ser autocontido. **scrcpy fica fora dos dois de propósito**: o pacote da distro Debian/Ubuntu é antigo demais (Ubuntu 24.04/Zorin 18 = 1.25, precisamos de >= 4.0) — `install.sh` checa a versão instalada e, com `--with-scrcpy`, baixa o build oficial do upstream; o Arch tem versão atual, então o PKGBUILD lista `scrcpy>=4.0` em `depends`. `install.sh` também tem `--check` (diagnóstico: binários, versões, udev, módulo, permissão do node, grupos do usuário) e `--uninstall`. `.deb` ganhou `postInstallScript`/`postRemoveScript` (`deb-postinst.sh`/`deb-postrm.sh`): recarrega udev, carrega o módulo, corrige o dono do node já existente e adiciona ao grupo `video` o usuário real por trás do `sudo`/`pkexec` — quem instala pela loja gráfica não precisa de terminal (SC-008/SC-010). **Rust**: `src/paths.rs` novo (`bin_dir_candidates`, 8 testes em `tests/paths_test.rs`) — `bundled_bin_dir()` procurava só `<exe>/bin/`, que é o layout do Windows; no `.deb` isso seria `/usr/bin/bin/` e o jar embutido nunca seria achado. Agora segue a mesma regra do `resource_dir()` do tauri-utils (`<exe>/../lib/CamLink/bin`, `$APPDIR/usr/lib/CamLink/bin`, `/usr/lib/CamLink/bin`). `detect_control_device_block()` em `v4l2.rs` (5 testes) traduz os dois modos de falha do instalador em erro acionável: EACCES → `v4l2_permission_denied` ("falta grupo video / logout"), ENOENT → `v4l2_module_not_loaded`. **Dois bugs reais encontrados ao inspecionar os pacotes, ambos afetando também o Windows do T067**: (1) o `.deb` (e o NSIS/MSI) empacotava `fake_backend`, o binário fake dos testes (T018), em `/usr/bin` — o bundler do Tauri empacota TODO `[[bin]]` do manifesto; corrigido com `required-features = ["fake-backend"]` no `[[bin]]` + dev-dependency do crate nele mesmo (`camlink = { path = ".", features = ["fake-backend"] }`), que liga a feature só em `cargo test` — `cargo build --release` não constrói e o bundler pula. (2) `run()` passava um `PathBuf::from("adb")` fixo pro `spawn_device_polling`, ou seja, PATH — ignorando o adb que o instalador embute: numa máquina limpa sem adb no PATH nenhum celular seria detectado apesar do binário estar instalado junto (FR-024). Agora usa `resolve_external_paths()`, que também passou a logar os 4 caminhos resolvidos na inicialização (diagnóstico pro T071). **Validado nesta máquina (Zorin 18 / base Ubuntu 24.04)**: `pnpm tauri build --bundles deb,appimage` gera `CamLink_0.1.0_amd64.deb` (5.5 MB) e `CamLink_0.1.0_amd64.AppImage`; `dpkg -c`/`dpkg -f` confirmam `/usr/bin/camlink` sozinho (sem `fake_backend`), `/usr/lib/CamLink/bin/scrcpy-server-camlink`, as regras em `/usr/lib/{udev/rules.d,modules-load.d,modprobe.d}` e o control com Depends/Section/Homepage/Description corretos; o AppImage rodado de verdade (via `AppRun`) loga adb, ffmpeg e jar resolvidos de dentro do bundle, não do PATH. Gates verdes (254 testes). **Não validado**: instalação limpa de verdade (`dpkg -i` em VM sem as dependências) e o PKGBUILD/AUR — não há Arch nesta máquina; ambos ficam pro T071. Nota: o `.deb` sai com o nome de pacote `cam-link` (kebab do `productName`), enquanto o PKGBUILD usa `camlink`.
- [x] T067 [P] Instalador Windows: NSIS/MSI via Tauri bundler com adb/scrcpy/ffmpeg embutidos + registro do filtro DirectShow próprio (`regsvr32` equivalente no instalador, sem driver de terceiros) em `installer/windows/` (FR-024) — **concluído (2026-08-19)**. `installer/windows/vendor.ps1` baixa adb+DLLs (platform-tools oficial do Google) e ffmpeg (build estático GPL do BtbN/FFmpeg-Builds, branch n8.1) pra `installer/windows/vendor/bin/` (gitignored) e copia o jar do fork já buildado (`scrcpy/dist/scrcpy-server-camlink`); no Windows o cliente `scrcpy.exe` NÃO é embutido (R12 — vídeo Android no Windows não passa pelo binário cliente). `src-tauri/tauri.windows.conf.json` (override Windows-only) mapeia esses binários + `camlink_lib.dll` (o próprio filtro DirectShow, já que o crate principal compila como `cdylib` — ver T023) via `bundle.resources`, e aponta `bundle.windows.nsis.installerHooks`/`bundle.windows.wix.fragmentPaths` pros hooks que chamam `regsvr32 /s`/`regsvr32 /u /s` no install/uninstall (`installer/windows/nsis-hooks.nsh`, `installer/windows/wix-fragment.wxs`). `resolve_external_paths()` (`lib.rs`) passou a checar `<exe>\bin\` (onde os `resources` são instalados) antes do fallback em PATH — não muda `cargo tauri dev` (sem instalador, cai direto no PATH como antes). **Validado em bancada pelo usuário**: `pnpm tauri build` gerou os dois instaladores (`CamLink_0.1.0_x64-setup.exe` 45.2MB, `CamLink_0.1.0_x64_en-US.msi` 61.6MB) e ambos foram instalados e testados com sucesso ("funcionou perfeitamente").
...
- [x] T085 Erros do backend de câmera virtual chegavam mudos à tela — dois bugs achados na primeira bancada real do T071 — **concluído (2026-09-23)**. Usuário testou o **AppImage** numa máquina sem os pré-requisitos e recebeu `falha do backend de câmera virtual: falha ao executar v4l2loopback-ctl: No such file or directory (os error 2)`. **(1) Falha de SPAWN não tinha diagnóstico**: `detect_secure_boot_block`/`detect_control_device_block` (T066) só rodam quando o `v4l2loopback-ctl` EXECUTA e falha, lendo o stderr dele. Com o binário ausente, o `Command` falha antes de produzir stderr e caía num `map_err` que formatava o `io::Error` cru. Ou seja: o PRIMEIRO erro que um usuário sem pré-requisitos encontra era o de PIOR mensagem. Novo `ctl_spawn_error` trata `ErrorKind::NotFound` com código `v4l2loopback_ctl_ausente` e dica apontando o `install.sh`/`v4l-utils`. **(2) Bug irmão, mais grave, achado ao investigar o primeiro**: `VcamError::Backend` carrega só `String`, e o código fazia `VcamError::Backend(err.msg)` — **descartando o `action_hint`**. Depois, `lib.rs` fazia `AppError::new("vcam_create_failed", e.to_string())`, sem dica. O frontend exibe `action_hint` desde sempre (`+page.svelte:412`), mas o texto nunca chegava lá: **todas as dicas acionáveis escritas no T066 eram construídas e jogadas fora** — grupo `video` faltando, módulo não carregado e Secure Boot chegavam mudos ao usuário. Corrigido com `VcamError::Actionable(AppError)` e um `vcam_error()` que preserva o erro diagnosticado nos 3 sítios de `vcam_create_failed`. 3 testes de regressão (257 no total). README: aviso do AppImage promovido a bloco de destaque — o texto anterior existia e foi pulado na prática — mais entrada no troubleshooting com a mensagem exata. **Terceiro achado, corrigido em seguida a pedido do usuário**: `V4l2Backend` não recebia caminhos configurados e usava `FFMPEG_BIN = "ffmpeg"` do PATH, ignorando o ffmpeg que o AppImage embute (`bundle.linux.appimage.files`, T066) e que `resolve_external_paths()` já sabia encontrar — mesma classe do bug do `adb` corrigido no T066. Numa máquina sem ffmpeg instalado a câmera virtual falharia apesar de o binário estar dentro do próprio pacote; não se manifestou nas bancadas porque todas as máquinas de teste têm ffmpeg. `V4l2Backend::new` passou a receber `PathBuf`, `spawn_ffmpeg` virou método, e `lib.rs` clona `external.ffmpeg` ANTES de mover `external` para o `StreamManager`. `Default` mantém o fallback de PATH, que é o correto onde não há resolução disponível (testes). 2 testes de regressão travando o comportamento — 259 no total. O `DShowBackend` ignora o parâmetro: o filtro DirectShow não spawna ffmpeg.
- [ ] T071 Instalação limpa validada: Ubuntu 22.04/24.04, Arch, Windows 10 e 11 — primeiro vídeo sem terminal (SC-008, SC-010) — **perna Arch do empacotamento validada (2026-09-22)**, em máquina Arch (Omarchy), que era o ambiente que faltava quando o T066 foi escrito. `makepkg` rodado de verdade e **4 defeitos reais do PKGBUILD corrigidos**: (1) `url=` apontava para `luizhfmonteiro/camlink`, que dá 404 — como as duas entradas de `source=()` derivam de `${url}`, o `makepkg` abortava no download (mesma URL errada estava no `homepage` do `tauri.conf.json`, que vai para o control do `.deb` e os metadados do MSI; corrigida junto); (2) `build()` rodava `cargo build --release` sem pôr o jar em `installer/linux/vendor/bin/`, então o `build.rs` do `tauri-build` abortava com `resource path '../installer/linux/vendor/bin/scrcpy-server-camlink' doesn't exist` — terceira aparição da mesma causa raiz no mesmo dia (as outras duas foram os jobs Windows e Linux do CI); corrigido usando o jar REAL, que já está no `srcdir` como fonte própria do pacote, em vez de um stub; (3) `makedepends` listava `rust` explícito, e no Arch o `rustup` declara `Conflicts With: rust cargo` — quem usa rustup não conseguiria buildar; trocado pelo virtual `cargo`, satisfeito por qualquer um dos dois; (4) as fontes dependem da tag `v0.1.0` e de um asset de release que ainda não existem — não é bug, é ordem: o PKGBUILD só fica testável de ponta a ponta quando a release existir, e é por isso que nunca tinha sido exercitado. **Como foi testado**: `source=()` apontado para um tarball local gerado por `git archive --prefix=CamLink-0.1.0/` (que reproduz o tarball do GitHub, inclusive a ausência do `vendor/` gitignored) + o jar local; `build()` e `package()` rodaram intactos. Build limpo do zero termina com exit 0. **Conteúdo do pacote conferido**: `/usr/bin/camlink` sozinho (sem `fake_backend`, confirmando que a correção do T066 vale também no Arch), jar em `/usr/lib/CamLink/bin/`, udev/modules-load.d/modprobe.d, `.desktop`, ícones e licença. **Validação mais importante**: o binário empacotado foi executado e logou `server_jar=…/usr/lib/CamLink/bin/scrcpy-server-camlink`, provando o `paths.rs` do T066 num pacote Arch real e não só nos 8 testes unitários; `adb`/`scrcpy`/`ffmpeg` caem no PATH, que é o correto aqui (vêm da distro via `depends`, ao contrário do AppImage). **Ainda pendente do T071**: `pacman -U` de verdade (exige sudo), `dpkg -i` em VM Ubuntu limpa, Windows 10/11 e o primeiro vídeo sem terminal com hardware. **Lacuna de paridade corrigida em seguida (2026-09-22)**: o `camlink.install` do Arch só IMPRIMIA a instrução `usermod -aG video`, enquanto o `deb-postinst.sh` já descobria o usuário real por trás do `sudo`/`pkexec` e resolvia sozinho — quem instalasse pelo AUR e não lesse a mensagem abriria o app e falharia ao criar a câmera virtual (`v4l2_permission_denied`), contrariando SC-008/SC-010 e a paridade Linux que o `.deb` já entregava. Alinhado: o hook do Arch agora conserta o dono de `/dev/v4l2loopback` se o módulo já estava carregado (a regra udev só vale pra device criado depois dela) e adiciona ao grupo o usuário de `SUDO_USER` (yay/paru/`sudo pacman`) ou `PKEXEC_UID` (pamac/gnome-software), caindo na instrução manual só quando não dá pra descobrir quem é. Testado com stubs nos 5 caminhos (sudo, polkit, shell de root puro, `SUDO_USER=root`, e usuário já no grupo — este último provando idempotência: `usermod` não é chamado). Confirmado embutido como `.INSTALL` no pacote gerado.
- [x] T072 [P] Documentação de usuário final (`README.md` completo: instalação, autorização USB, troubleshooting/diagnósticos da tabela do quickstart) — **concluído (2026-09-22)**. README reorganizado para usuário final: Instalação (`.deb`, AUR, AppImage, Windows NSIS/MSI), Usando com o celular Android (habilitar depuração USB, autorizar, iniciar), Usando uma câmera IP (RTSP), e Solução de problemas com 10 subseções cobrindo os 5 casos da tabela de diagnósticos do quickstart (Secure Boot, scrcpy < 4.0, Android < 12, Firefox/v4l2compat, RAW simultâneo) mais os erros reais do backend (não autorizado, sem permissão no grupo video, módulo não carregado, nenhum celular na lista). Stack e instruções de build saíram do topo para uma seção Contribuindo no fim — é conteúdo de quem contribui, não de quem usa. **Quatro erros factuais meus pegos ao verificar contra o código, e não escritos de memória**: (1) eu tinha escrito `cam-link_0.1.0_amd64.deb` como nome do arquivo, mas `cam-link` é o nome do PACOTE dentro dele — o arquivo é `CamLink_0.1.0_amd64.deb`; (2) a amostra da saída do `install.sh --check` estava inventada (`udev rule: instalada`), troquei pelo formato real do script, que imprime o caminho; (3) eu mandava o usuário do AppImage rodar `./installer/linux/install.sh`, mas quem baixa só o AppImage não tem o repositório — e o script depende de TRÊS arquivos irmãos (rules/modules-load/modprobe), então baixá-lo sozinho também não funciona; instrução corrigida para clonar o repo, com a ressalva de que `.deb`/AUR não exigem isso; (4) eu afirmava que havia campos separados de usuário e senha no painel RTSP, mas só existe campo de Senha — conforme `inject_credentials`, o usuário vai na URL (`rtsp://user@host`) OU o par `usuario:senha` inteiro vai no campo de senha. Âncoras internas verificadas por script (o GitHub PRESERVA acentos nos slugs, então `#limitacoes-conhecidas` estava quebrado e virou `#limitações-conhecidas`). **Lacuna de usabilidade identificada, não corrigida**: o AppImage exige clonar um repositório git para configurar o v4l2loopback, o que contraria a proposta de autocontido — candidato a embutir o setup no próprio AppImage (escopo de T066/T071).
- [ ] T083 [P] Revisão de UI/UX do frontend (SvelteKit): conceito visual/design já está bom, mas execução não — não está responsivo (layout quebra em janelas menores/redimensionadas) e algumas partes não estão legíveis (contraste/tamanho de texto). **Adicionado a pedido do usuário (2026-08-19).** **Causa raiz investigada e parte objetiva corrigida (2026-09-22)**, medindo o app rodando em Chromium headless, não por leitura de código. **(1) O layout transbordava 40px na horizontal em QUALQUER largura** — não só em janela pequena, onde apenas ficava evidente. Causa: o projeto não tinha reset, `box-sizing` era `content-box` e o `<body>` mantinha a `margin: 8px` do user-agent, então `.layout { width: 100%; padding: 1.5rem }` resolvia para 48px a mais que o container (medido: viewport 800px, `scrollWidth` 840px, 9 elementos fora da borda). Corrigido com reset global; medido depois: overflow 0px, 0 elementos fora. **(2) Dropdowns claros na UI escura**: `color-scheme` nunca foi declarado, então o WebKit/Chromium desenha todo controle nativo (o `<select>` e o popup de `<option>`) em modo claro; somado a `background: transparent`, que deixava a UA pintar o próprio fundo, e a `color: inherit`, que puxava o `--muted` do `<label>` — o valor escolhido saía em cor secundária a 60%, invertido (o rótulo é que é secundário). Corrigido com `color-scheme: light dark`, `background: var(--card-bg)` e um token `--text` novo. **(3) O select de balanço de branco não recebia estilo nenhum**: vive em `CameraControls.svelte` e a regra estava no `<style>` escopado do `+page.svelte`. A regra virou `:global(select)`, uma definição só para os cinco dropdowns do app. **(4)** `minWidth`/`minHeight` (640×520) na janela, que abria em 800×600 sem mínimo — abaixo do único breakpoint do app (900px) — contra cards de 336px fixos. **(5)** `<title>` do documento ainda era o boilerplate `Tauri + SvelteKit + Typescript App`. **DUAS HIPÓTESES DO ENUNCIADO ORIGINAL FORAM REFUTADAS ao medir**: (a) *contraste* — `--muted` PASSA no WCAG AA (4.64:1 no claro, 5.95:1 no escuro), não é a causa da ilegibilidade; (b) *composição de `em`* — os 43 `font-size` em `em` abaixo de 1 pareciam encolher multiplicativamente, mas nenhum wrapper de nenhum componente define `font-size`, então todo `em` resolve contra os 16px da raiz e se comporta como `rem`. Medido no app: 0 elementos abaixo de 12px. A conversão `em`→`rem` seria refactor sem efeito visual e foi deixada de fora. **Cards fluidos implementados a pedido do usuário (2026-09-22)**: `.source-grid` saiu de `flex-wrap` com cards de `width: 336px` fixo para `grid-template-columns: repeat(auto-fill, minmax(min(21rem, 100%), 1fr))` — cada card ocupa no mínimo 21rem (os 336px de antes) e cresce para preencher a faixa. O `min(21rem, 100%)` é o que impede o transbordo em janela mais estreita que 21rem: sem ele o `minmax` mantém o mínimo e o card estoura. `SourceCard` perdeu a largura fixa (o `.thumb` já usa `aspect-ratio: 16/9`, então a miniatura acompanha) e o `.ghost-card` trocou `height: 260px` por `min-height: 16.25rem`. Todas as dimensões de layout viraram `rem`, incluindo o breakpoint (`max-width: 56.25rem`) — media query em `rem` respeita a preferência de tamanho de fonte do usuário, `px` não. Verificado em 1400/900/640/420px: sem transbordo em nenhuma, inclusive a 420px, bem abaixo do `minWidth` de 640. **Seletor manual de tema implementado (2026-09-22, decisão do usuário)**: o app sempre soube renderizar claro e escuro, mas só obedecendo o `prefers-color-scheme` do SO — não havia como escolher. Agora há um seletor Claro/Escuro/Sistema na topbar (`ThemeToggle.svelte` + `theme.ts`), persistido em localStorage. Contrato com o CSS: atributo `data-theme` no `<html>` — ausente = segue o SO, `light`/`dark` = fixado. O bloco de tokens escuros é DUPLICADO de propósito (`:root[data-theme="dark"]` e `@media (prefers-color-scheme: dark) :root:not([data-theme="light"])`), porque uma das situações vive dentro de media query. **A alternativa enxuta seria `light-dark()`, com um token só, e foi descartada conscientemente**: é Baseline 2024 e o WebKitGTK do Ubuntu 22.04 (alvo do quickstart e do T071) é antigo demais — se não suportar, TODOS os tokens caem de uma vez. Script inline em `app.html` aplica o tema salvo antes do primeiro paint, senão a página pinta no tema do SO e pisca. Todo acesso a localStorage é try/catch (janela privada, storage bloqueado) com fallback para o comportamento antigo de seguir o SO. **Verificado no app rodando**: os três estados de `data-theme` renderizam corretamente com SO escuro (light sobrepõe o SO, dark fixa, ausente segue); clicar nos três botões atualiza atributo + localStorage + `color-scheme` + fundo; e após reload com "Escuro" salvo o atributo já vem aplicado e o botão certo ativo — provando o anti-flash e a restauração de estado. Menor texto do app hoje é `.status-pill`/`.url` a 11,5px — marginal, também decisão de design.
- [x] T084 [P] Mecânica de release: `.github/workflows/release.yml` disparado por tag `v*` + `CHANGELOG.md` — **concluído (2026-09-22)**. Não estava no plano original; virou necessário porque os instaladores eram buildados à mão em cada plataforma, e o Windows dependia de alguém rodar `vendor.ps1` + `pnpm tauri build` numa máquina Windows. Cinco jobs: `versao` (rejeita tag que não bate com a versão do `tauri.conf.json`, evitando publicar um pacote v0.2.0 que por dentro se chama 0.1.0), `jar` (builda o `scrcpy-server-camlink` do fork com JDK 17 + Android SDK e sobe como artefato — os jobs de plataforma não precisam de Java nem do SDK), `linux` (.deb + AppImage), `windows` (NSIS + MSI) e `release`. O jar também vira asset por si só, porque é de lá que o PKGBUILD do AUR baixa (`releases/download/vX.Y.Z/scrcpy-server-camlink`) em vez de exigir Android SDK na máquina de quem instala. **A release sai como RASCUNHO de propósito**: o workflow compila e empacota, mas não instala nada — publicar continua sendo decisão humana depois do T071. `workflow_dispatch` permite exercitar o pipeline inteiro sem criar tag. **Endurecimentos feitos a partir de teste local, não de suposição**: (1) o upload usa `mapfile` em array em vez de `$(find ...)` solto — testado com nome contendo espaço, que na forma antiga viraria dois argumentos; (2) o caminho do `sdkmanager` é localizado com `find` em vez de assumir `cmdline-tools/latest`, que varia entre imagens de runner; (3) a lógica da checagem de versão foi exercitada com `v0.1.0` (aceita) e `v0.2.0` (rejeita). **Não exercitado de ponta a ponta**: nenhuma tag foi empurrada, então o pipeline completo — build do jar no runner, vendoring real com download de ~150 MB, bundling nas duas plataformas — ainda não rodou. O `workflow_dispatch` existe justamente para isso.
- [ ] T073 Passe final de gates + quickstart completo nas duas plataformas; auditoria: sem `unwrap()` em caminho falível, `// SAFETY:` em todo unsafe FFI (Constituição VI); auditoria de privacidade (FR-026): confirmar que nenhuma dependência/código faz chamada de rede além das fontes RTSP configuradas, e documentar no README — **auditoria de `unsafe` do `dshow.rs` concluída (2026-09-22)**. Números reais: 49 sítios `unsafe` (não os "38" de um grep de linhas), 15 documentados, 34 descobertos. Agora 49/49. **Destravou uma capacidade que faltava**: `rustup target add x86_64-pc-windows-msvc` + stubs locais de `installer/windows/vendor/` permitem rodar `cargo clippy --target x86_64-pc-windows-msvc -- -D warnings` no Linux — até aqui o código Windows só era verificável no CI. (De quebra, o `build.rs` falhando por `bundle.resources` ausente foi a QUARTA aparição da mesma causa raiz no dia.) **A auditoria não foi só documentação: achou três classes de defeito real.** **(1) Leitura fora de limites vinda de memória compartilhada** — `FrameHub::read_if_newer` fazia `from_raw_parts(data_ptr, header.frame_len)` com `frame_len` lido da região compartilhada, ou seja de FORA do processo. `write_frame` limita a `FRAME_MAX_BYTES`, mas o leitor não pode depender disso: um escritor de outra versão ou uma região corrompida faria a leitura passar do view mapeado. A guarda de `push_sample` (`rgb24.len() != expected`) não protegia — ela roda depois, a cópia já aconteceu. Corrigido com clamp no leitor. **(2) `CoTaskMemAlloc` sem checagem de null** em 4 sítios; o único que checava usava `assert!`. Num filtro DirectShow isso é especialmente grave: o código roda DENTRO do processo consumidor (`obs64.exe`, `chrome.exe`) e o pânico atravessando a fronteira COM vira abort — derrubaria o programa do usuário por uma falha de alocação nossa. `build_media_type_for` passou a devolver `Option`, com `alloc_media_type_copy` novo centralizando a cópia, e os chamadores devolvem `E_OUTOFMEMORY`. **(3) Out-params do COM desreferenciados sem checar null**, de forma inconsistente — o arquivo já checava `pcfetched`, `pppins` e `pscc`, mas não `ppmediatypes` (cujo irmão `SinglePinEnum::Next` checava!), `pmt`, `picount`/`pisize`, `ppmt`, `ppropdata` nem `pinfo`. Todos passaram a devolver `E_POINTER`, que é a convenção COM. Achado colateral corrigido junto: `IEnumMediaTypes::Next` marcava `*done = true` ANTES das alocações, então uma falha de memória "queimava" o enumerador e o consumidor nunca mais veria o formato. **Não testado em hardware**: as correções são verificadas por compilação cruzada e revisão; o caminho COM real precisa da bancada Windows do T071. **Auditoria de privacidade (FR-026) concluída (2026-09-22)**, em cinco frentes. **Dependências**: das 274 crates da árvore, NENHUMA é cliente HTTP, stack TLS ou SDK de telemetria; as únicas com capacidade de rede são `mio` e `socket2`, primitivas de I/O do tokio. Nenhum plugin de updater do Tauri declarado. **Sockets do código**: exatamente dois, ambos loopback — `TcpStream::connect(("127.0.0.1", forward_port))` em `stream_manager.rs` e `ControlClient::connect(("127.0.0.1", port))` em `lib.rs`, que são os túneis do adb para o celular via USB. `ControlClient::connect` aceita `ToSocketAddrs` genérico, mas o único chamador passa loopback (verificado). **Frontend**: zero `fetch`/`XMLHttpRequest`/`WebSocket`/`@import`/`<link>` remoto; `font-family: Inter, …` é referência a fonte LOCAL com fallback, não Google Fonts; as URLs `svelte.dev/e/...` no bundle são links de documentação dentro de strings de erro, não recursos buscados. **Subprocessos**: adb/scrcpy/v4l2loopback-ctl são locais; o ffmpeg tem dois usos — entrada rawvideo local e entrada RTSP com a URL digitada PELO USUÁRIO (o caso permitido), com saída sempre local (`-f v4l2` ou rawvideo no stdout), sem push remoto. **Credenciais/logs**: keyring usa Secret Service (DBus local) e Credential Manager; `tracing` escreve em stdout sem sink de rede; existe `redact_url` censurando credencial antes de logar. **Lacuna encontrada e corrigida**: `"csp": null`. Nada carregava recurso externo, mas não havia MECANISMO impedindo — a FR-026 estava garantida por disciplina. Definida CSP restritiva (`default-src 'self'`, `object-src/frame-src/worker-src 'none'`, `connect-src` limitado ao IPC do Tauri, `img-src`/`media-src` com `data:` por causa dos frames do preview em base64). **VERIFICADA NO APP REAL, não só por gate**: risco era o HTML gerado ter dois `<script>` inline (o bootstrap do SvelteKit e o anti-flash de tema do T083) e `script-src 'self'` bloqueá-los, cujo sintoma seria tela branca que nenhum gate pega. Build release rodado e janela capturada com `grim`: a UI renderizou completa, o IPC funcionou (apareceu "Nenhum dispositivo Android detectado", resposta real de `list_devices`) e zero violações no log. **Efeito colateral valioso**: a captura confirmou que os dropdowns do T083 renderizam escuros no WEBKITGTK — até então o `color-scheme` só tinha sido testado em Chromium, ressalva que fica resolvida. Documentado no README em seção "Privacidade" (exigência da própria T073). **Passe de `unwrap()/expect()` concluído (2026-09-22)**: os 63 sítios foram classificados e verificados um a um. **Resultado: NENHUM está em caminho falível** — a exigência da constituição já era cumprida, só nunca tinha sido verificada. Distribuição: **45** são `Mutex::lock().unwrap()` (propagação de poison — o modo de falha é "outra thread entrou em pânico segurando o lock", ou seja um bug que já aconteceu; propagar é o idioma padrão do Rust e não é caminho falível no sentido da constituição); **8** em módulos `#[cfg(test)]`; **9** `expect()` com invariante, todas verificadas individualmente: `raw_manager.rs:102` (`DNG_LEN_FIELD = 8` com guarda `buf.len() < dng_len_end` imediatamente antes, o slice é provadamente de 8 bytes), `raw_manager.rs:120` (`RawFrameMetadata` deriva `Serialize` em struct simples, `to_vec` é infalível), `rtsp_manager.rs:318` (o laço roda `max_attempts.max(1)` vezes e só chega ali com ao menos um `Err` registrado), `secrets.rs:19` (poison com mensagem descritiva), e as duas de inicialização em `lib.rs` (`Runtime::new()` e o build do Tauri — sem runtime ou sem app não há o que degradar). **Nota de método**: 2 sítios que o classificador automático marcou como `.unwrap()` "nu" no `dshow.rs` eram `.lock().unwrap()` quebrados em várias linhas — contagem automática de `unwrap` engana, a leitura é que decide. **Única mudança de código**: as 4 ocorrências de `ctx.control.as_mut().expect("ensure_control garantiu")` viraram `control_mut(ctx)?`. O `expect` era são (todo caminho `Ok` de `ensure_control` preenche `ctx.control` — verificado), mas dependia de uma invariante a ~70 linhas de distância, repetida em 4 chamadores; agora ela é explícita e um refactor futuro que a quebre devolve `AppError` acionável em vez de derrubar o app. **Ainda pendente do T073**: o quickstart completo nas duas plataformas, que depende da bancada do T071.
